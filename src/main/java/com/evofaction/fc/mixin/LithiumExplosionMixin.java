...
// Need to override this for protection blocks. Everything else is the same.
// Disables lithium's world.explosions - See fabric.mod.json
// https://github.com/CaffeineMC/lithium/blob/9d3d6bbcbc8703ef39f03ee44912b09604b4877b/src/main/java/me/jellysquid/mods/lithium/mixin/world/explosions/ExplosionMixin.java#L1
/**
...
public abstract class LithiumExplosionMixin {
    ...
    public boolean collectBlocks(ObjectArrayList<BlockPos> affectedBlocks, Collection<BlockPos> collection) {
        // FabricCannoning - Skip collecting blocks entirely if explosion was in a protection block column
    ...
    private float traverseBlock(float strength, int blockX, int blockY, int blockZ, LongOpenHashSet touched) {
        ...
        if (reducedStrength > 0.0F && (this.explodeAirBlocks || !blockState.isAir())) {
            ...
            if (this.behavior.canDestroyBlock((Explosion) (Object) this, this.world, pos, blockState, reducedStrength)) {
                // FabricCannoning - Don't explode protected blocks
                if (!Config.PROTECTION_BLOCK_ENABLED || !ProtectionBlock.isProtectedColumn(this.world, MathHelper.floor(blockX), MathHelper.floor(blockZ)))
